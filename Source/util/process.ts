	...
	static async exec(
		...
		try {
			process.onStdErr((data) => {
				stderrBytesWritten += stderrBuffer.write(
					data,
					stderrBytesWritten,
				);
			});
			...
			process.onStdOut((data) => {
				stdoutBytesWritten += stdoutBuffer.write(
					data,
					stdoutBytesWritten,
				);
			});
						...
						if (options?.onCancellation) {
							try {
								...
								if (cancellationOptions.waitForProcessClose) {
									let processCloseListener: () => void =
										() => {
											return;
										}; // Not required, but makes the compiler happy.

									const timeoutMs =
										cancellationOptions.waitForProcessCloseTimeout ||
										60 * 1000; // 1 minute default timeout for any process to shutdown.
									const processClosePromise =
