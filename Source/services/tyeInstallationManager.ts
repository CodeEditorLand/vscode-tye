...
export default class LocalTyeInstallationManager
	implements TyeInstallationManager
{
	private readonly satisfiedVersions = new Set<string>();

	constructor(
		private readonly expectedVersion: string,
		private readonly tyeCliClient: TyeCliClient,
	...
	async ensureInstalledVersion(
		...
		if (!isVersionInstalled) {
				...
				context.buttons = [
					{
						...
						title: localize(
							"services.tyeInstallationManager.installLatestTitle",
							"Install Latest Tye",
						),
					},
				];
			...
			throw new Error(
				localize(
					"services.tyeInstallationManager.versionNotInstalled",
					"A compatible version of Tye has not been found. You may need to install a more recent version.",
				),
			);
		}
	}
	...
	async isVersionInstalled(version: string): Promise<boolean> {
		...
		try {
			...
			if (
				semver.satisfies(cliVersion, version, {
