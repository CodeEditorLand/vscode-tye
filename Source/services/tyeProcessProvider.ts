...
function tyeProcessesComparer(x: TyeProcess[], y: TyeProcess[]): boolean {
	return arrayComparer(
		x,
		y,
		(a: TyeProcess, b: TyeProcess) => a.pid - b.pid,
		tyeProcessComparer,
	);
}

function tyeProcessWithPortsComparer(
	x: TyeProcessWithPorts,
	y: TyeProcessWithPorts,
): boolean {
	return (
		x.pid === y.pid &&
		arrayComparer(
...
function tyeProcessesWithPortsComparer(
	x: TyeProcessWithPorts[],
	y: TyeProcessWithPorts[],
): boolean {
	return arrayComparer(
		x,
		y,
		(a: TyeProcessWithPorts, b: TyeProcessWithPorts) => a.pid - b.pid,
		tyeProcessWithPortsComparer,
	);
}
...
export default class LocalTyeProcessProvider implements TyeProcessProvider {
	...
	constructor(
		...
		this._processes =
			...
			timer(0, 2000).pipe(
				...
				switchMap((processes) =>
					this.determineDashboardPorts(processes),
				),
	...
	private async getTyeProcessesWithPorts(): Promise<TyeProcessWithPorts[]> {
		...
		return await Promise.all(
			tyeProcesses.map((process) =>
				this.getTyeProcessWithPorts(process.pid),
			),
		);
	}

	private async determineDashboardPorts(
		processes: TyeProcessWithPorts[],
	): Promise<TyeProcess[]> {
		const allProcesses = await Promise.all(
			processes.map((process) => this.determineDashboardPort(process)),
		);

		function isValidProcess(
			process: TyeProcess | undefined,
		): process is TyeProcess {
			return !!process;
		}
	...
	private async getTyeProcessWithPorts(
		pid: number,
	): Promise<TyeProcessWithPorts> {
		const ports = await this.portProvider.getPortsForProcess(pid);

		return { pid, ports };
	}

	private async determineDashboardPort(
		process: TyeProcessWithPorts,
