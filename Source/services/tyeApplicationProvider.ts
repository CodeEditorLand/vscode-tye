...
export function applicationComparer(
	x: TyeApplication | undefined,
	y: TyeApplication | undefined,
): boolean {
	...
	for (let j = 0; j < xServices.length; j++) {
		...
		if (
			xijService.name !== yijService.name ||
...
export class TaskBasedTyeApplicationProvider implements TyeApplicationProvider {
	...
	constructor(
		private readonly tyeProcessProvider: TyeProcessProvider,
		private readonly tyeClientProvider: TyeClientProvider,
	) {
		this._applications = tyeProcessProvider.processes.pipe(
			switchMap((processes) => this.toApplicationsStream(processes)),
		);
	}
	...
	async getApplications(): Promise<TyeApplication[]> {
		const firstApplications = await this.applications
			.pipe(first())
			.toPromise();
	...
	private toApplicationsStream(
		processes: TyeProcess[],
	): Observable<TyeApplication[]> {
	...
	private async toApplications(
		processes: TyeProcess[],
	): Promise<TyeApplication[]> {
		const applications = processes
			.map((process) => ({
				dashboard: vscode.Uri.parse(
					`http://localhost:${process.dashboardPort}`,
				),
				pid: process.pid,
			}))
			.map((process) => ({
				...process,
				tyeClient: this.tyeClientProvider(process.dashboard),
			}))
