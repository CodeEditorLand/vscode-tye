...
export interface TyeClient {
	...
	getServices(
		token?: vscode.CancellationToken
	): Promise<TyeService[] | undefined>;

	shutDown(token?: vscode.CancellationToken): Promise<void>;
}

export type TyeClientProvider = (
	dashboard: vscode.Uri | undefined
) => TyeClient | undefined;

export class HttpTyeClient implements TyeClient {
	constructor(
		private readonly httpClient: HttpClient,
		private readonly apiEndpoint = "http://localhost:8000/api/v1"
	) {}
	...
	public async getLog(
		...
		const resp = await this.httpClient.get(
			`${this.apiEndpoint}/logs/${serviceName}`,
			token
		);
	...
	public static fromBaseEndpoint(
		httpClient: HttpClient,
		fromBaseEndpoint: vscode.Uri
	): HttpTyeClient {
		return new HttpTyeClient(
			httpClient,
			vscode.Uri.joinPath(fromBaseEndpoint, "api", "v1").toString()
		);
	}
}

export function httpTyeClientProvider(
	httpClient: HttpClient
): TyeClientProvider {
	return (dashboard) => {
