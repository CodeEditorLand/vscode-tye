...
export class TyeDebugConfigurationProvider
	implements vscode.DebugConfigurationProvider
{
	...
	async resolveDebugConfigurationWithSubstitutedVariables(
		...
		function isValidApplication(
			a: TyeApplication | undefined
		): a is TyeApplication {
			return a !== undefined;
		}

		function allServicesRunning(a: TyeApplication): boolean {
			return Object.values(a.projectServices).every(
				(service) => Object.values(service.replicas).length
			);
		}
		...
		try {
			application = await this.userInput.withProgress(
				localize(
					"debug.tyeDebugConfigurationProvider.waitingForApplication",
					"Waiting for Tye application to start..."
				),
				(progress, cancellationToken) => {
					const cancellation =
						observableFromCancellationToken<TyeApplication>(
							cancellationToken
						);

					const applicationMonitor =
						this.tyeApplicationProvider.applications.pipe(
							map((applications) =>
								applications.find(
									(a) =>
										a.name.toLowerCase() ===
										tyeDebugConfiguration.applicationName.toLowerCase()
								)
							),
		...
		if (!application) {
			throw new Error(
				localize(
					"debug.tyeDebugConfigurationProvider.applicationNotRunning",
					'The Tye application "{0}" is not running.',
					tyeDebugConfiguration.applicationName
				)
			);
		}

		const debuggableServiceNames = Object.keys(
			application.projectServices ?? {}
		);

		if (
			application.projectServices === undefined ||
			debuggableServiceNames.length === 0
		) {
			throw new Error(
				localize(
					"debug.tyeDebugConfigurationProvider.noDebuggableServices",
					'The Tye application "{0}" does not have any debuggable services.',
					tyeDebugConfiguration.applicationName
				)
			);
		}

		const debuggedServiceNames =
			tyeDebugConfiguration.services ?? debuggableServiceNames;

		if (debuggedServiceNames.length === 0) {
			throw new Error(
				localize(
					"debug.tyeDebugConfigurationProvider.noDebuggedServices",
					"No services were set to be debugged."
				)
			);
		}
		...
		for (const serviceName of debuggedServiceNames) {
			...
			if (service === undefined) {
				throw new Error(
					localize(
						"debug.tyeDebugConfigurationProvider.noDebuggedService",
						'The Tye application "{0}" does not have the debugged service "{1}".',
						tyeDebugConfiguration.applicationName,
						serviceName
					)
				);
		...
		if (tyeDebugConfiguration.watch) {
			this.tyeApplicationWatcher.watchApplication(application.id, {
				folder,
				services: tyeDebugConfiguration.services,
			});
		}
