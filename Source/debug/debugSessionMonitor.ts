	...
	implements DebugSessionMonitor
{
	...
	constructor() {
		...
		this.onDidStartListener = vscode.debug.onDidStartDebugSession(
			(debugSession) => {
				if (
					debugSession.type === coreClrDebugSessionType &&
					debugSession.configuration.request ===
						coreClrDebugRequestType &&
					debugSession.configuration.processId
				) {
					this.pids.add(
						parseInt(
							<string>debugSession.configuration.processId,
							10,
						),
					);
				}
