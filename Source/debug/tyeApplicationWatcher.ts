...
export interface TyeApplicationWatcher {
	watchApplication(
		...
		options?: { folder?: vscode.WorkspaceFolder; services?: string[] },
	): void;
}

export class TyeApplicationDebugSessionWatcher
	extends vscode.Disposable
	implements TyeApplicationWatcher
{
	...
	constructor(
		...
		super(() => {
			for (const watchedApplicationId of Object.keys(
				this.watchedApplications,
			)) {
					...
					if (application) {
						for (const serviceName of Object.keys(
							...
							if (
								...
								const service =
									application.projectServices[serviceName];

								for (const replicaName of Object.keys(
									service.replicas,
								)) {
									const currentPid =
										service.replicas[replicaName];

									void attachToReplica(
