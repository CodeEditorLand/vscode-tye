...
async function getFunctionProcess(pid: number): Promise<string> {
	...
	const functionProcess = processes
		.slice()
		.reverse()
...
export async function attachToReplica(
	debugSessionMonitor: DebugSessionMonitor,
	folder: vscode.WorkspaceFolder | undefined,
	serviceType: KnownServiceType,
	replicaName: string,
	replicaPid: number | undefined,
): Promise<void> {
	if (
		replicaPid !== undefined &&
		!debugSessionMonitor.isAttached(replicaPid)
	) {
		...
		if (actualPid !== undefined) {
			await vscode.debug.startDebugging(folder, {
				type: "coreclr",
				name: localize(
					"debug.attachToReplica.sessionName",
					"Tye Replica: {0}",
					replicaName,
				),
				request: "attach",
