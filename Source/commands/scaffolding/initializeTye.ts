...
async function promptToOverwriteTyeYamlIfNecessary(
	folder: vscode.WorkspaceFolder,
	ui: UserInput,
): Promise<boolean> {
	const existingTyeYamlFiles = await vscode.workspace.findFiles(
		new vscode.RelativePattern(folder, "tye.{yml,yaml}"),
	);

	if (existingTyeYamlFiles.length > 0) {
		const overwrite: vscode.MessageItem = {
			title: localize(
				"commands.scaffolding.initTye.overwrite",
				"Overwrite",
			),
		};
...
export async function initializeTye(
	...
	if (!folders || folders.length === 0) {
		...
		throw new Error(
			localize(
				"commands.scaffolding.initializeTye.noFolder",
	...
	const shouldInvokeTyeInit = await promptToOverwriteTyeYamlIfNecessary(
		folder,
		ui,
	);
...
const createInitializeTyeCommand =
	(
		tyeApplicationConfigurationProvider: TyeApplicationConfigurationProvider,
		tyeCliClient: TyeCliClient,
		tyeInstallationManager: TyeInstallationManager,
