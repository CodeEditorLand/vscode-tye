...
async function createUniqueName(
	...
	if (name.done) {
		throw new Error(
			localize(
				"commands.scaffoldTyeTasks.uniqueNameError",
...
export async function scaffoldTyeTasks(
	context: IActionContext,
	configurationProvider: TyeApplicationConfigurationProvider,
	...
	if (configurations.length === 0) {
		...
		throw new Error(
			localize(
				"commands.scaffolding.scaffoldTyeTasks.noTyeYaml",
				"No Tye YAML file exists in the open workspace, or no workspace or folder has been opened.",
			),
		);
	}
	...
	if (configurations.length > 1) {
		...
		throw new Error(
			localize(
				"commands.scaffolding.scaffoldTyeTasks.tooManyTyeYamls",
				"Scaffolding supports only a single Tye YAML file per workspace.",
			),
		);
	}
	...
	const onTaskConflict: ConflictHandler = async (label, isUnique) => {
		const overwrite: vscode.MessageItem = {
			title: localize(
				"commands.scaffolding.scaffoldTyeTasks.overwriteTask",
				"Overwrite",
			),
		};
		...
		const result = await ui.showWarningMessage(
			localize(
				"commands.scaffolding.scaffoldTyeTasks.taskExists",
		...
		if (result === overwrite) {
			...
			label = await createUniqueName(
				localize(
					"commands.scaffolding.scaffoldTyeTasks.taskPrefix",
					"{0}-",
					label,
				),
				isUnique,
	...
	await scaffolder.scaffoldConfiguration(
		localize(
			"commands.scaffolding.scaffoldTyeTasks.configurationName",
			"Debug with Tye",
		),
		workspaceConfiguration.folder,
		(name) => {
		...
		async (name, isUnique) => {
			const overwrite: vscode.MessageItem = {
				title: localize(
					"commands.scaffolding.scaffoldTyeTasks.overwriteConfiguration",
					"Overwrite",
				),
			};
			...
			const result = await ui.showWarningMessage(
				localize(
					"commands.scaffolding.scaffoldTyeTasks.configurationExists",
			...
			if (result === overwrite) {
				...
				name = await createUniqueName(
					localize(
						"commands.scaffolding.scaffoldTyeTasks.configurationPrefix",
						"{0} - ",
						name,
					),
					isUnique,
...
const createScaffoldTyeTasksCommand =
	(
		configurationProvider: TyeApplicationConfigurationProvider,
		scaffolder: Scaffolder,
		ui: UserInput,
	...
	(context: IActionContext): Promise<void> =>
		scaffoldTyeTasks(context, configurationProvider, scaffolder, ui);
