...
const tyeAppStartedRegex = new RegExp(
	"started successfully with Pid: (?<pid>[0-9]+)",
);
			...
			(name, definition, callback) => {
				...
				return telemetryProvider.callWithTelemetry(
					...
					async (context: IActionContext) => {
						await tyeInstallationManager.ensureInstalled(
						...
						const command = CommandLineBuilder.create(
							...
							.withNamedArg(
								"--framework",
								tyeDefinition.framework,
							)
							.withNamedArgs("--tags", tyeDefinition.tags)
							.withNamedArg(
								"--verbosity",
								tyeDefinition.verbosity,
							)
						...
						await callback(command, {
							cwd: definition.cwd,
							onCancellation:
								async (): Promise<ProcessCancellationOptions> => {
									if (tyePid !== undefined) {
										const applications =
										...
										if (application) {
											const tyeClient = tyeClientProvider(
											...
											if (tyeClient) {
												...
												return {
													...
													waitForProcessCloseTimeout:
														tyeProcessShutdownTimeout,
												};
											}
										}
									}
