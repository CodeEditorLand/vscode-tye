...
import {
	WorkspaceTyeApplicationConfigurationProvider,
	YamlTyeApplicationConfigurationReader,
} from "./services/tyeApplicationConfiguration";
import {
	KnownServiceType,
	TaskBasedTyeApplicationProvider,
...
export function activate(context: vscode.ExtensionContext): Promise<void> {
		...
		(actionContext: IActionContext) => {
			...
			registerDisposable(
				vscode.workspace.registerTextDocumentContentProvider(
					"tye-log",
					registerDisposable(
						new TyeLogDocumentContentProvider(
			...
			const treeProvider = new TyeServicesTreeDataProvider(
				tyeApplicationProvider,
				tyeClientProvider,
				tyeInstallationManager,
				ui,
			);
			...
			telemetryProvider.registerCommandWithTelemetry(
				...
				async (context, node: TyeApplicationNode) => {
					if (
						node.application.dashboard?.scheme === "http" ||
						node.application.dashboard?.scheme === "https"
					) {
						await vscode.env.openExternal(
							node.application.dashboard,
						);
					}
				},
			);
			...
			const debugSessionMonitor = registerDisposable(
				new CoreClrDebugSessionMonitor(),
			);

			telemetryProvider.registerCommandWithTelemetry(
				"vscode-tye.commands.attachService",
				async (context, node: TreeNode) =>
					await registerAttachCommand(node, debugSessionMonitor),
			);
			...
			telemetryProvider.registerCommandWithTelemetry(
				...
				createInitializeTyeCommand(
					tyeApplicationConfigurationProvider,
					tyeCliClient,
					tyeInstallationManager,
					ui,
				),
			...
			telemetryProvider.registerCommandWithTelemetry(
				...
				async (context, node: TyeServiceNode) => {
					const logUri = vscode.Uri.parse(
						`tye-log://logs/${node.application.id}/${node.service.description.name}`,
					);
				...
				async (context, node: TyeApplicationNode) => {
					...
					if (application?.projectServices) {
						for (const service of Object.values(
							...
							for (const replicaName of Object.keys(
								service.replicas,
							)) {
			...
			const applicationWatcher = registerDisposable(
				new TyeApplicationDebugSessionWatcher(
					debugSessionMonitor,
					tyeApplicationProvider,
				),
			);
			...
			registerDisposable(
				vscode.debug.registerDebugConfigurationProvider(
					"tye",
					new TyeDebugConfigurationProvider(
						debugSessionMonitor,
						tyeApplicationProvider,
						applicationWatcher,
						ui,
	...
	async function registerAttachCommand(
		node: TreeNode,
		debugSessionMonitor: CoreClrDebugSessionMonitor,
	) {
		const replicas: {
			replica: TyeReplica;
		...
		if (node instanceof TyeServiceNode && isAttachable(node.service)) {
			replicas.push(
				...Object.values(node.service.replicas).map((replica) => ({
					replica: replica,
					serviceType: <KnownServiceType>node.service.serviceType,
				})),
			);
		} else if (
			node instanceof TyeReplicaNode &&
			isAttachable(node.service)
		) {
			replicas.push({
