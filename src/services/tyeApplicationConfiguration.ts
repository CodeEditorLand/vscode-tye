...
export class YamlTyeApplicationConfigurationReader
	implements TyeApplicationConfigurationReader
{
	readConfiguration(content: string): Promise<TyeApplicationConfiguration> {
...
export class WorkspaceTyeApplicationConfigurationProvider
	implements TyeApplicationConfigurationProvider
{
	constructor(
		private readonly configurationReader: TyeApplicationConfigurationReader
	) {}
	...
	async getConfigurations(): Promise<ProvidedTyeApplicationConfiguration[]> {
		...
		return files.map((file) => ({
			...
			getConfiguration: async () => {
				...
				let configuration =
					await this.configurationReader.readConfiguration(
						contentString
					);
